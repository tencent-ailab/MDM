...
if __name__ == '__main__':
    ...
    def train(it):
        ...
        if 'full' in config.model.network:
            ...
            # ))
            logger.info('[Train] Iter %05d | Loss %.2f | Loss(Global) %.2f | Loss(Local) %.2f | Loss(node_global) %.2f | Loss(node_local) %.2f |Grad %.2f | LR %.6f' % (
                    it, loss.item(), loss_global.mean().item(), loss_local.mean().item(), loss_node_global.mean().item(), loss_node_local.mean().item(), orig_grad_norm, optimizer_global.param_groups[0]['lr'],
                ))
